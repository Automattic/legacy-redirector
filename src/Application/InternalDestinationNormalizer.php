 ...
 * the list table - apart from the escapes whose decoded form would mean
 * something else (see Url::decode()), and keeps the query percent-encoded:
 * query values have sub-structure, so decoding a literal '%26' into '&'
 * would turn one value into two parameters. WordPress re-encodes non-ASCII
 * on the way out through wp_sanitize_redirect(), so a decoded stored path
 * still emits a valid Location header.
 *
...
final class InternalDestinationNormalizer {
	...
	/**
	 * Punctuation that is decoded in a destination's path.
	 *
	 * Narrower than Url::PATH_TEXT, because a destination is sent back out
	 * through wp_sanitize_redirect(), which strips characters such as '$' and
	 * "'" rather than encoding them; decoding those would change where the
	 * visitor lands. '+' is kept too: WordPress gives it its own meaning in
	 * some paths, '/tag/one+two/' among them.
	 */
	private const string PATH_TEXT = '-._~!*(),;=:@&';
	...
	private function assemble( string $encoded_path, ?string $query, ?string $fragment ): ?string {
		$path = Url::decode( '' === $encoded_path ? '/' : $encoded_path, self::PATH_TEXT )
			. ( null !== $query ? '?' . $query : '' )
			. ( null !== $fragment ? '#' . Url::decode( $fragment, self::PATH_TEXT . '/?' ) : '' );
