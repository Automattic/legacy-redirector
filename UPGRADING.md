...
`wp legacy-redirector migrate --dry-run` shows them before anything is written, and `wp legacy-redirector migrate --list-conflicts` lists every one at any time afterwards, with the redirect it collided with. Review them, then either delete them or re-point and re-enable them; each drops off that list once saved.
