...
- The plugin is renamed from "WPCOM Legacy Redirector" to "Legacy Redirector". The `WPCOM` prefix was a misnomer: the plugin is not specific to WordPress.com and runs on any WordPress install. The text domain is now `legacy-redirector` and the `X-Redirect-By` header now reads `Legacy Redirector`. Four things deliberately keep the old name because changing them would break working installs for no user benefit: the main plugin file stays `wpcom-legacy-redirector.php` so existing installs stay active through the upgrade, the Composer package stays `automattic/wpcom-legacy-redirector` so nobody has to edit their `require` line, and the `vip-legacy-redirect` post type and the `wpcom_legacy_redirector_db_version` family of options stay as they are because they hold live data.
...
- The WP-CLI command set has been redesigned. The old `wp wpcom-legacy-redirector` namespace is kept as a deprecated alias for `find-domains` and `import-from-meta` only; commands added in 2.0 are not aliased, and calling a removed command fails with an error that names its replacement. See [UPGRADING.md](UPGRADING.md) for the full old-to-new command mapping: `insert-redirect` is now `create` (validating by default), `import-from-csv` is now `import`, `export-to-csv` has been removed in favor of `list --format=csv`, and `import-from-meta` flags are now kebab-case.
...
- Redesigned WP-CLI command set: `create`, `get`, `list`, `update`, `delete`, `enable`, `disable`, `validate`, `import`, `find-domains`, and `migrate`. Every command accepts a redirect ID or a source path, and the write commands accept multiple redirects at once.
  ...
  - `list --duplicates` lists the redirects the migration disabled because, once normalized, they share a source with a live redirect that points somewhere else, with both destinations side by side. It marks those whose spelling no browser could ever have requested under 1.x, which can simply be deleted, and the admin shows the same list as a Duplicate sources view, with the choices spelled out. A disabled duplicate can't be enabled while the live redirect has its source.
  - `delete`, `enable`, and `disable` take multiple redirects behind a single confirmation (or `--yes`).
  - `validate` runs in batch or targeted mode (`wp legacy-redirector validate /path 123`), disables broken redirects with `--fix`, and exports broken ones with `--format=csv`. Destination checks use `wp_safe_remote_get()`/`wp_safe_remote_head()`, so stored URLs cannot be used to probe loopback, private, or reserved addresses (SSRF); validating destinations on other internal hosts requires opting in via the `http_request_host_is_external` filter, as described in the README. `--check-source` requests each source and reads the first response without following it, catching a source that serves its own content and one that redirects somewhere other than its destination; a failed request is a warning, so it never gets a redirect disabled by `--fix`.
  - `import` replaces `import-from-csv`, reads from STDIN (`import -`), supports `--mode=upsert` and `--verbose`, and round-trips a status column so enabled/disabled state survives export and import. A leading header row whose first column is `from` is skipped, so `list --fields=from,to,status --format=csv` output can be imported unedited.
...
- Warnings for redirect sources on paths WordPress itself serves (`/wp-admin`, `/wp-login.php` and the other root `wp-*.php` files, `/xmlrpc.php`, and anything under `/wp-json`, `/wp-content` or `/wp-includes`). Such a redirect lies dormant while the path works but answers it the moment it 404s, which for `/wp-admin` can lock you out of the dashboard. `create` and `import` warn, the Add/Edit Redirect form warns as soon as the source field loses focus (before anything is saved) and again whenever such a redirect is opened, and `validate` reports them (without disabling them under `--fix`). They are not refused, since a migrated site can have genuine legacy URLs there.
...
- Source paths containing non-ASCII or percent-encoded characters are matched correctly, in https://github.com/Automattic/legacy-redirector/pull/102, including where home is not the domain root — a source saved for `example.com/日本/ページ` resolves for the percent-encoded path a browser actually requests. The spellings of one path now find the same redirect, where 1.x compared the text left by `esc_url_raw()`, and the migration re-keys existing sources so a 1.x redirect stored encoded, or with a `+` or a space, keeps working. Escapes whose decoded form would change the meaning stay distinct (`%2F`, `%3F`, `%23` and `%25`, plus `%26`, `%3D` and `%2B` in a query string), and a `+` in a path is a literal plus, so `/tag/one+two` is not confused with `/tag/one two`. Matching no longer depends on the server's locale either: every URL the plugin parses now goes through a UTF-8 safe parser, where PHP's own `parse_url()` corrupts raw multibyte bytes on hosts whose `LC_CTYPE` treats the C1 range as control characters.
