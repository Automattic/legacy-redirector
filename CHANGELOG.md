...
- External redirect destinations now work. They are accepted at creation time from all entry points, and the destination host is automatically allowed at redirect time; 1.x passed every destination through `wp_safe_redirect()`, so redirects to hosts outside `allowed_redirect_hosts` silently did nothing.
- Source paths containing non-ASCII or percent-encoded characters are matched correctly, in https://github.com/Automattic/wpcom-legacy-redirector/pull/102, including where home is not the domain root — a source saved for `example.com/日本/ページ` resolves for the percent-encoded path a browser actually requests.
