...
- Redesigned WP-CLI command set: `create`, `get`, `list`, `update`, `delete`, `enable`, `disable`, `validate`, `import`, `find-domains`, and `migrate`. Every command accepts a redirect ID or a source path, and the write commands accept multiple redirects at once.
  ...
  - `list --duplicates` lists the redirects the migration disabled because, once normalized, they share a source with a live redirect that points somewhere else, with both destinations side by side.
...
- Source paths containing non-ASCII or percent-encoded characters are matched correctly, in https://github.com/Automattic/legacy-redirector/pull/102, including where home is not the domain root — a source saved for `example.com/日本/ページ` resolves for the percent-encoded path a browser actually requests. Every spelling of one path now finds the same redirect, where 1.x matched only the exact text stored, and the migration re-keys existing sources so a 1.x redirect stored encoded, or with a `+` or a space, keeps working. Escapes whose decoded form would change the meaning stay distinct (`%2F`, `%3F`, `%23` and `%25`, plus `%26`, `%3D` and `%2B` in a query string), and a `+` in a path is a literal plus, so `/tag/one+two` is not confused with `/tag/one two`. Matching no longer depends on the server's locale either: every URL the plugin parses now goes through a UTF-8 safe parser, where PHP's own `parse_url()` corrupts raw multibyte bytes on hosts whose `LC_CTYPE` treats the C1 range as control characters.
